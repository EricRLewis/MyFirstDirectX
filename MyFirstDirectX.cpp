...
Author: Eric Lewis
Date: 2/21/2015
MyFirstDirectX.cpp: First project for DirectX fun. Built for fun. Feel free to destroy and rebuild. 
*/
...
float newX;							   // x coordinate of ther mouse on the current frame
float newY;							   // y coordinate of ther mouse on the current frame
	...
	switch (message)
	{
		...
		case WM_MOUSEMOVE:
		{
			...
			if (wParam == MK_CONTROL){
				...
				newX = GET_X_LPARAM(lParam)/400.0 - 1;
				newY = GET_Y_LPARAM(lParam)/ -400.0 + 1;
			}
...
{
	//Set up mouse coordinate global variables for initial frame
	oldX = 0;
	oldY = 0;
	newX = 0;
	newY = 0;
